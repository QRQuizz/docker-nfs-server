# registry.tools.flinkdrop.com/nfs-server
...
Starting the `registry.tools.flinkdrop.com/nfs-server` image will launch an NFS server. You'll need to supply some information upon container startup, which we'll cover below, but briefly speaking your `docker run` command might look something like this:
...
1. **Provide your desired [NFS exports](https://linux.die.net/man/5/exports) (`/etc/exports`)**
   ...
   1. bake `/etc/exports` into a custom image
       ...
       ```Dockerfile
       FROM registry.tools.flinkdrop.com/nfs-server
...
1. **Use `--cap-add SYS_ADMIN` or `--privileged`**
   ...
   As noted in the [requirements](#requirements), the container will need additional privileges. So your `run` command will need *either*:
       ...
       docker run --cap-add SYS_ADMIN ... registry.tools.flinkdrop.com/nfs-server
       
    or

       docker run --privileged ... registry.tools.flinkdrop.com/nfs-server
...
1. **Expose the server ports**
   ...
   * If your clients connect via **NFSv4 only**, you can get by with just TCP port `2049`:
         ...
         docker run -p 2049:2049 ... registry.tools.flinkdrop.com/nfs-server
